...
static const char *vertex_shader = "#version 450 core\n"
  ...
  "void main() {\n"
    "if(gl_VertexID == 0 || gl_VertexID == 5){\n"
      "gl_Position =   projection * vec4((quad_pos[gl_VertexID]*scale_vec * scale_factor) + pos_coords, 0.0, 1.0);\n"
      "tex_out = tex_coords;\n"
    "}\n"
  ...
  for(;*current != 0;current++){
    ...
    switch (*current){
      ...
      default:
        ...
        add_char_info(current, glo_x + xoff , glo_y +  yoff); 
...
unsigned int create_centered_text(const char *copy_text, float cx, float cy, float width
  ...
  if(width_font > height_font){
    resize_font(height_font);
    glCheckError();
    return create_text(copy_text, cx - (calc_width*font_size)/2, cy - height/2);
  }
  else{
    resize_font(width_font);
    glCheckError();
    
    return create_text(copy_text, cx - width/2, cy - (CURRENT_FONT.line_height * font_size)/2);
  }
}

unsigned int create_text(const char *copy_text, float glo_x, float glo_y){
  vs();

  texts.insert(std::pair<unsigned int, Text>(index_hash,{NULL,0,0, 0,0,0}));
  ...
  glCheckError();
  add_string_info(current.text, glo_x, glo_y);
...
void modify_text(unsigned int text_id, const char *new_text){
  ...
  add_string_info(new_text, current.x,current.y);
...
void array_from_texts(unsigned int total_size){
  ...
  for(auto &current : texts){
    auto &data = current.second;
    add_string_info(data.text, data.x, data.y);
  }
...
void print_vbo_debug() {
    ...
    if(mapped_data) {
        ...
        for(size_t i = 0; i < num_elements; i += CHAR_RENDER_SIZE) {
            ...
            printf("pos(%.2f, %.2f)\n", mapped_data[i+5], mapped_data[i+6]);
